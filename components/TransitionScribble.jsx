...
export default function TransitionScribble() {
    useEffect(() => {
        ...
        const runScribbleAnimation = (e, options = { scrollToTop: true, text: null, color: null }) => {
            ...
            const randomColor = options.color || transitionColors[Math.floor(Math.random() * transitionColors.length)];
            transitionScribbleSvg.style.color = randomColor;
            document.documentElement.style.setProperty('--current-transition-color', randomColor);
            ...
            document.documentElement.style.setProperty('--current-transition-text-color', logoColor);
            ...
            drawTl.to(transitionLogo, {
                ...
                onStart: () => {
                    const innerEl = transitionLogo.firstChild;
                    if (innerEl && innerEl.tagName === 'IMG') {
                        gsap.to(innerEl, { rotation: 5, duration: 0.15, repeat: -1, yoyo: true, ease: 'steps(1)', overwrite: 'auto' });
                    }
                }
            }, durIn * 0.5);
